...
<link rel="manifest" href="%(r)smanifest.webmanifest">
<meta name="theme-color" content="#6b4785">
<meta name="mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="default">
<meta name="apple-mobile-web-app-title" content="BHR Eng">
...
%(js)s
<script>if('serviceWorker' in navigator){addEventListener('load',function(){navigator.serviceWorker.register('%(r)ssw.js').catch(function(){})})}</script>
</body>
</html>
...
**Read `MAINTAINING.md`.** It says where every kind of content lives, how to
add an assignment, a link or a full brief, and what has already been checked so
you do not redo it.
...
def build_repo_files():
    ...
    # the maintenance guide is authored by hand, not generated; the build only
    # carries it through so it survives a rebuild
    _m = os.path.join(SRC, 'MAINTAINING.md')
    if os.path.exists(_m):
        write('MAINTAINING.md', open(_m, encoding='utf-8').read())
...
def write_pwa():
    """manifest.webmanifest + sw.js at the site root, so the site installs as
    an app on a phone and opens offline. Everything is relative, so it works
    at https://user.github.io/repo/ as well as at a domain root. The cache
    name carries a hash of every page and asset, so each rebuild is a new
    cache and phones pick up changes on their next open."""
    import json, hashlib
    manifest = {
        "name": "BHR Engineering Technology",
        "short_name": "BHR Eng",
        "description": "Blue Hills Regional Engineering Technology: the shop, the projects, the pathways.",
        ...
        "icons": [
            ...
            {"src": "assets/icon-512-maskable.png", "sizes": "512x512",
    ...
    for root, dirs, files in os.walk(OUT):
        ...
        if top in ('_source', 'files', 'posters'):
            dirs[:] = []
            continue
        for fn in sorted(files):
            fp = os.path.join(root, fn)
            if fn.endswith(('.html', '.css', '.js', '.png', '.jpg', '.webmanifest')):
                h.update(open(fp, 'rb').read())
                url = './' + os.path.relpath(fp, OUT).replace(os.sep, '/')
                if url in ('./sw.js',):
                    continue
                shell_urls.append(url)
    ver = h.hexdigest()[:10]
    sw = r"""/* BHR Engineering service worker -- generated by build_site.py, do not edit.
...
""" % (ver, json.dumps(shell_urls))
    with open(os.path.join(OUT, 'sw.js'), 'w', encoding='utf-8') as f:
        f.write(sw)
    print('  %-42s %6d shell files, cache bhr-eng-%s' % ('sw.js + manifest', len(shell_urls), ver))
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(SRC):
        ...
        for f in files:
            ...
            if f in SKIP_DIRS or f == '.last-shipped.json' or f == 'WHAT-CHANGED.txt' or os.path.islink(fp) or not os.path.isfile(fp):
                continue
